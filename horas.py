...
JSON_PATH = os.environ.get("JSON_PATH", os.path.join(os.path.dirname(__file__), "clasificacion_proyectos.json"))
SHEET_SALIDA = "IPI"
...
if __name__ == "__main__":
    if not GUI_ENABLED:
        raise SystemExit("Tkinter no disponible. Ejecuta en GUI local o usa la versión web.")
    if not os.path.exists(JSON_PATH):
        Persist().save()
    App().mainloop()
