...
JSON_PATH = os.path.join(os.path.dirname(__file__), "clasificacion_proyectos.json")
SHEET_SALIDA = "IPI"
...
class Persist:
    ...
    @classmethod
    def load(cls):
        if not os.path.exists(JSON_PATH): return cls()
        try:
            with open(JSON_PATH,"r",encoding="utf-8") as f: d=json.load(f)
            obj = cls()
...
def open_as_xlsx(path: str):
    ...
    if ext==".xls":
        # 1) pandas + xlrd (si está disponible); si falla, COM
        try:
            import pandas as pd
            ...
            try:
                ...
                raise RuntimeError("Para .xls: usa pandas+xlrd o Excel (pywin32).") from e
...
def find_all_proyectos_positions(ws)->List[int]:
    ...
    for r in range(1, ws.max_row+1):
        for c in range(1, ws.max_column+1):
            if RE_PROYECTOS_TAG.match(read_cell(ws,r,c) or ""): res.append(r); break
    return res
    ...
    for r in range(1, min(80, ws.max_row)+1):
        ...
        while c <= ws.max_column:
            ...
            if v.isdigit() and int(v) == 1:
                ...
                c += 1
    return 31, 4  # fallback razonable

# ----------------- Parseo del bloque -----------------

def parse_block(ws, r1, r2, recurso, n_days, day_start)->List[RowData]:
    """
    Extrae SOLO filas de imputación entre r1..r2 para un recurso.
    - Proyecto en col 1 o 2.
    - Tipo en la col siguiente de la misma fila o en col 1/2/3 de filas siguientes.
    - Anti-resumen: el índice N de "N-HORA ..." debe crecer por proyecto; si baja, se ignoran
      las filas restantes hasta el siguiente proyecto.
    - Las horas se leen en columnas absolutas: desde day_start durante n_days.
    """
    rows=[]
    ...
    for r in range(r1, r2+1):
        ...
        if proj:
            ...
            if tipo:
                ...
                seq_max = idx  # primer índice aceptado aunque empiece en 2 ó 3
                horas=[read_cell(ws,r,c) for c in range(day_start, day_start+n_days)]
                rows.append(RowData(recurso=recurso,
...
def style_header(ws, row_idx, n_days):
    ...
    for c,t in enumerate(headers,1):
        cell=ws.cell(row=row_idx,column=c,value=t); cell.fill=fill; cell.font=bold; cell.alignment=align; cell.border=border
    ws.column_dimensions["A"].width=36; ws.column_dimensions["B"].width=48; ws.column_dimensions["C"].width=22
...
def style_row(ws, row_idx, color, n_cols):
    ...
    for c in range(1,n_cols+1):
        cell=ws.cell(row=row_idx,column=c); cell.fill=fill; cell.border=border

def build_output(wb_out, rows: List[RowData], persist: Persist, n_days:int):
    if SHEET_SALIDA in wb_out.sheetnames: del wb_out[SHEET_SALIDA]
    ...
    for rd in rows:
        ...
        vals = rd.horas_por_dia[:n_days]  # ya alineado por día absoluto
        for i,v in enumerate(vals, start=4): ws.cell(row=r0, column=i, value=v or "")
        col_total = 3+n_days+1; col_total_dec=3+n_days+2; col_tipo=3+n_days+3
    ...
    sumas: Dict[str,int]={}
    for rd in rows:
        if not rd.tipo_imputacion: continue
    ...
    for rd in rows:
        if not rd.tipo_imputacion: continue
        if rd.tipo_proyecto not in sum_tipo: continue
...
def collect_discovered(recursos: List[str], proyectos: Dict[str,str], persist: Persist):
    ...
    for cod, nom in proyectos.items():
        if cod not in persist.tipos: persist.tipos[cod]=""; changed=True
        if persist.nombres.get(cod)!=nom: persist.nombres[cod]=nom; changed=True
...
def process_file(input_path: str, persist: Persist)->str:
    ...
    if not pos: raise RuntimeError("No se encontró 'Proyectos:'")

    n_days, day_start = detect_day_grid(ws)

    bloques=[]
    for i,r in enumerate(pos):
        r1=r; r2=pos[i+1]-2 if i<len(pos)-1 else ws.max_row
        if r1<=r2: bloques.append((r1,r2))
    all_rows=[]; recursos=[]; proyectos={}
    for (r1,r2) in bloques:
        recurso = extract_recurso_line(ws,r1) or "RECURSO DESCONOCIDO"
        if recurso not in recursos: recursos.append(recurso)
    ...
    # exclusiones
    all_rows=[rd for rd in all_rows if rd.recurso not in persist.excluir_recursos and (rd.proyecto_codigo is None or rd.proyecto_codigo not in persist.excluir_proyectos)]
...
class App(tk.Tk):
    ...
    def on_browse(self):
        p=filedialog.askopenfilename(title="Selecciona Excel", filetypes=[("Excel","*.xlsx *.xls"),("Todos","*.*")])
    ...
    def on_edit_clasif(self):
        ...
        def save_close():
            for cod in proys:
                if vars_con[cod].get() and not vars_rep[cod].get(): self.persist.tipos[cod]="CONSTRUCCION"
                elif vars_rep[cod].get() and not vars_con[cod].get(): self.persist.tipos[cod]="REPARACION"
                else: self.persist.tipos[cod]=""
...
def main():
    if tk is None: raise SystemExit("Tkinter no disponible.")
    if not os.path.exists(JSON_PATH): Persist().save()
    App().mainloop()
